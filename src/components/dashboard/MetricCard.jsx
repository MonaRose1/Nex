...
const MetricCard = ({ title, value, subtext, type = 'default' }) => {
    ...
    return (
        <div className="p-6 bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 relative overflow-hidden group">
            {/* Decorative glow */}
            ...
            <h3 className="text-white/50 text-xs font-bold uppercase tracking-widest mb-4 relative z-10">{title}</h3>

            <div className="relative z-10">
                <div className={`text-3xl font-sora font-bold mb-2 ${isTeal ? 'text-brightTeal drop-shadow-[0_0_10px_rgba(60,178,184,0.3)]' : 'text-white'
                    }`}>
                    {value}
                </div>
                <p className="text-white/30 text-xs font-inter uppercase tracking-wide">{subtext}</p>
            </div>
